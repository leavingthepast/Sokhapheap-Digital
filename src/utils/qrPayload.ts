...
export function createCompactPatientPayload(patient: Patient): string {
  const compact = {
    ...
    em: patient.emergencyContact || { name: 'Emergency Contact', relationship: 'Family', phone: '' },
    t: patient.qrToken,
    tc: patient.qrTokenCreatedAt,
    alg: (patient.allergies || []).filter(Boolean).map(a => ({ id: a.id, n: a.name || '', r: a.reaction, s: a.severity })),
    vac: (patient.vaccinations || []).filter(Boolean).map(v => ({ id: v.id, n: v.name || '', d: v.date, p: v.provider, nt: v.notes })),
    ill: (patient.illnessHistory || []).filter(Boolean).map(h => ({ id: h.id, c: h.condition, d: h.diagnosedDate, s: h.status, h: h.doctorOrHospital, nt: h.notes })),
    lab: (patient.labResults || []).filter(Boolean).map(l => ({ id: l.id, t: l.testName, d: l.date, r: l.result, ref: l.referenceRange, s: l.status, h: l.labOrHospital })),
    rec: (patient.medicalRecords || []).filter(Boolean).map(r => ({
      id: r.id,
      n: r.name || '',
      tp: r.type,
      d: r.date,
