...
export const DoctorMedicalRecordView: React.FC<DoctorViewProps> = ({ patient, onExit }) => {
  ...
  const filteredRecords = useMemo(() => {
    if (!searchQuery) return (patient.medicalRecords || []).filter(Boolean);
    const q = searchQuery.toLowerCase();
  ...
  const filteredLabs = useMemo(() => {
    if (!searchQuery) return (patient.labResults || []).filter(Boolean);
    const q = searchQuery.toLowerCase();
  ...
  return (
    <div 
      className="min-h-screen bg-[#f8fafc] text-slate-900 font-sans pb-20 select-none"
      ...
      <header className="bg-white border-b border-slate-200 sticky top-0 z-40 shadow-xs">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-3.5 flex flex-wrap items-center justify-between gap-4">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 flex items-center gap-2">
                ...
                <span className="inline-flex items-center gap-1 text-[10px] font-bold text-amber-800 bg-amber-50 border border-amber-200 px-2 py-0.5 rounded-full">
                  ...
                  <span>View Only (Protected Mode)</span>
                </span>
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div 
              className="inline-flex items-center gap-1.5 px-3 py-2 bg-amber-50 border border-amber-200 text-amber-900 text-xs font-bold rounded-xl select-none"
              title="Clinical document downloading, exporting, and printing are strictly disabled for scan views"
            >
              <Lock className="w-3.5 h-3.5 text-amber-700" />
              <span className="hidden sm:inline">Protected View-Only</span>
            </div>
                ...
                {patient.emergencyContact?.name ? (
                  <div className="flex items-center gap-2 pt-1">
                    ...
                    >
                      ...
                      <span>{patient.emergencyContact.name} ({patient.emergencyContact.relationship || 'Contact'}) — {patient.emergencyContact.phone || 'No phone'}</span>
                    </a>
                  </div>
                ) : null}
              </div>
